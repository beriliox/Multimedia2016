<?php
  ...
  if(!$image_perfil){
    $image_perfil = 'views/app/img/administrador.png';
  }

  $sql = $db->query("SELECT nombre FROM Administrador WHERE nombre='$nombres' OR email='$correo' OR rut='$rut' LIMIT 1;");
  if($db->rows($sql) == 0) { // si no existe?

      $db->query("INSERT INTO Administrador (rut, dv, nombre, apellidop, apellidom, direccion, ciudad, email, image_perfil, estado, pass)
                  VALUES ('$rut','$dv','$nombres','$apellidop','$apellidom','$direccion','$ciudad','$correo','$image_perfil','Activo', '$pass');");
      $HTML = 1;
    } else {
    $administrador = mysqli_fetch_array($sql)[0];
    if(strtolower($nombres) == strtolower($administrador)){
